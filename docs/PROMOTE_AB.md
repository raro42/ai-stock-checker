...
3. **Window B (promote on):** same length, same capital baseline (reset or note starting equity), same fees/gates except promote. Desk promote A/B glance warns `B blocked · …` when live knobs drift from protocol max 5 / 24h hold / `revolut_standard` (or open count > 5) via `window_b_readiness` — restore knobs before starting B.

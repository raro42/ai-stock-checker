...
3. **Window B (promote on):** same length, same capital baseline (reset or note starting equity), same fees/gates except promote. Desk promote A/B glance warns `B blocked · …` when live `max_positions` or open count drifts from protocol max 5 (`window_b_readiness`) — restore caps before starting B.

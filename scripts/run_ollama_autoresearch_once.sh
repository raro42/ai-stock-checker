...
# Refuses daytime runs unless OLLAMA_AUTOSEARCH_FORCE=1 (night window: 23:00–08:00 Europe/Berlin).
set -euo pipefail
...
if [[ "$(python3 -m stock_checker.autoresearch_schedule in_window)" != "1" ]]; then
  ...
  echo "hint: wait for 23:00 Europe/Berlin, or set OLLAMA_AUTOSEARCH_FORCE=1 for a manual daytime run"
  exit 0
fi

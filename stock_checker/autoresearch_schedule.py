"""CEST night window for Ollama autoresearch (no daytime GPU/CPU burn).

Default window: 23:00 ≤ t < 08:00 Europe/Berlin (handles CEST/CET via zoneinfo).
Override with OLLAMA_AUTOSEARCH_NIGHT_START / _NIGHT_END / _TZ, or force with
OLLAMA_AUTOSEARCH_FORCE=1.
"""
...
def night_window_bounds() -> tuple[int, int, str]:
    ...
    tz_name = os.getenv("OLLAMA_AUTOSEARCH_TZ", "Europe/Berlin")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args or args[0] in ("-h", "--help"):
        print(
            ...
            "{in_window|seconds_until_open|status}",
            file=sys.stderr,
        )
        return 2
    cmd = args[0]

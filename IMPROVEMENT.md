...
- [x] **A2** Promote A/B: method + baseline + **Window A (promote off) started** 2026-08-12 — [docs/PROMOTE_AB.md](docs/PROMOTE_AB.md) / [docs/history/promote_ab_2026-08-12.md](docs/history/promote_ab_2026-08-12.md) — **2026-09-13:** Window A fee-adjusted fill summarize done; **2026-09-14:** Window B readiness blockers (max pos / open / hold / fee / soft gates / AI mode / multi-role / scan·trade floors) on desk glance; **2026-09-15:** glance bit shows fee-adjusted **net** (realized − all fees), not gross sell P&L; **2026-09-16:** Window A sample-size honesty — day target + ≥10 fills (`window_a_sample_readiness` / `A thin · N fills <10`); dual sample progress `N/10 fills` + `building sample` while days still run; fills≥10 before day target → `fills ready · keep Window A` (not bare `running`); buy/sell sides `Nb/Ns` + `A open-only · 0 sells` when ledger is all buys (fee-adjusted edge needs closes); Window B + fee-adjusted verdict still pending
...
- [ ] Optional later: staskh refuse-without-vol / hard rr_gate as live entry block (watch 2026-09-12 ema_vix · 2026-09-16 rr_gate) — desk soft-n/a coverage + Book Vol marks + live RR ok/thin/hit + designed RR stated on Exits/crypto glances shipped above; defer hard refuse until promote unlock / gate thinning allows
...
- [ ] Optional later: start Window B (promote on) after restoring protocol knobs (max 5 / 24h / revolut_standard / regime·RS·breadth on / AI validate / multi-role on / scan ≥15m / trade ≥5m) **and** Window A sample ≥10 fills **with ≥1 sell** — Window A summarize + B-readiness + hold/fee/gate/AI/multi-role/cadence knob parity + fee-adjusted **net** bit + A thin fill-floor + dual `N/10 fills` / `building sample` + early `fills ready · keep Window A` + buy/sell `Nb/Ns` + `A open-only` shipped 2026-09-13/16; do not flip compose default-on (A3)

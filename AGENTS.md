...
2. **Filter junk**: no stablecoins, leveraged tokens, or known noise symbols (`symbol_filters.py`). Live crypto **scan + buys** are majors only (BTC/ETH), max one crypto slot, ±10% exits (`crypto_policy.py`). Equity universe includes curated **German Xetra (`.DE`)** names; session hours use Xetra for `.DE` and US RTH otherwise (`market_hours.py`).
...
- **Prefer local Ollama** overnight to save Cursor tokens:
  `./scripts/run_ollama_autoresearch_loop.sh` (see `autoresearch/README.md`) — **23:00–08:00 Europe/Berlin only** (day idle; `OLLAMA_AUTOSEARCH_FORCE=1` for manual daytime)

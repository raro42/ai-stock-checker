...
| **Earnings blackout** | No new stock entries within ~2d before / 1d after Yahoo earnings on the **America/New_York date** (not UTC); **empty Yahoo window → allow (suspect)** / missing calendar → allow (fail-open), not a silent waiver (`earnings_guard.py`) |

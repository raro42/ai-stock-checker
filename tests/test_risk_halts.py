...
def test_entry_post_tp_mark_returns_tp_vs_oth_vs_fresh() -> None:
    from stock_checker.risk_halts import (
        book_risk_report,
        entry_post_tp_mark_returns,
    )
    ...
    pm = entry_post_tp_mark_returns(holds, trades)
    assert pm["entry_post_tp_tp_pct"] == -1.0
    assert pm["entry_post_tp_tp_label"] == "−1.0%×1"
    assert pm["entry_post_tp_tp_lots"] == 1
    assert pm["entry_post_tp_oth_pct"] == 2.0
    assert pm["entry_post_tp_oth_label"] == "+2.0%×1"
    assert pm["entry_post_tp_oth_lots"] == 1
    assert pm["entry_post_tp_fresh_pct"] == 3.5
    assert pm["entry_post_tp_fresh_label"] == "+3.5%×1"
    assert pm["entry_post_tp_fresh_lots"] == 1
    assert pm["entry_post_tp_unknown_lots"] == 1
    assert pm["entry_post_tp_marks_ready"] is True
    assert "tp −1.0%×1" in pm["entry_post_tp_marks_bit"]
    assert "oth +2.0%×1" in pm["entry_post_tp_marks_bit"]
    assert "fresh +3.5%×1" in pm["entry_post_tp_marks_bit"]

    empty = entry_post_tp_mark_returns([], [])
    assert empty["entry_post_tp_marks_ready"] is False
    assert empty["entry_post_tp_marks_bit"] == ""

    no_ledger = entry_post_tp_mark_returns(
        [
            {
                "symbol": "GOOG",
                "kind": "stock",
    ...
    assert no_ledger["entry_post_tp_fresh_pct"] == 4.0
    assert no_ledger["entry_post_tp_tp_lots"] == 0
    ...
    assert out["entry_post_tp_marks_ready"] is True
    assert out["entry_post_tp_tp_pct"] == -1.0
...
def test_entry_post_trim_mark_returns_trim_vs_oth_vs_fresh() -> None:
    from stock_checker.risk_halts import (
        book_risk_report,
        entry_post_trim_mark_returns,
    )
    ...
    pm = entry_post_trim_mark_returns(holds, trades)
    assert pm["entry_post_trim_trim_pct"] == -0.8
    assert pm["entry_post_trim_trim_label"] == "−0.8%×1"
    assert pm["entry_post_trim_trim_lots"] == 1
    assert pm["entry_post_trim_oth_pct"] == 2.0
    assert pm["entry_post_trim_oth_label"] == "+2.0%×1"
    assert pm["entry_post_trim_oth_lots"] == 1
    assert pm["entry_post_trim_fresh_pct"] == 3.5
    assert pm["entry_post_trim_fresh_label"] == "+3.5%×1"
    assert pm["entry_post_trim_fresh_lots"] == 1
    assert pm["entry_post_trim_unknown_lots"] == 1
    assert pm["entry_post_trim_marks_ready"] is True
    assert "trim −0.8%×1" in pm["entry_post_trim_marks_bit"]
    assert "oth +2.0%×1" in pm["entry_post_trim_marks_bit"]
    assert "fresh +3.5%×1" in pm["entry_post_trim_marks_bit"]

    empty = entry_post_trim_mark_returns([], [])
    assert empty["entry_post_trim_marks_ready"] is False
    assert empty["entry_post_trim_marks_bit"] == ""

    no_ledger = entry_post_trim_mark_returns(
        [
            {
                "symbol": "GOOG",
                "kind": "stock",
    ...
    assert no_ledger["entry_post_trim_fresh_pct"] == 4.0
    assert no_ledger["entry_post_trim_trim_lots"] == 0
    ...
    assert out["entry_post_trim_marks_ready"] is True
    assert out["entry_post_trim_trim_pct"] == -0.8
...
def test_entry_buy_source_mark_returns_scan_rebal_other_none() -> None:
    from stock_checker.risk_halts import (
        book_risk_report,
        entry_buy_source_mark_returns,
    )
    ...
    pm = entry_buy_source_mark_returns(holds, trades)
    assert pm["entry_buy_src_scan_pct"] == -0.5
    assert pm["entry_buy_src_scan_label"] == "−0.5%×1"
    assert pm["entry_buy_src_scan_lots"] == 1
    assert pm["entry_buy_src_rebal_pct"] == 2.5
    assert pm["entry_buy_src_rebal_label"] == "+2.5%×1"
    assert pm["entry_buy_src_rebal_lots"] == 1
    assert pm["entry_buy_src_other_pct"] == 1.0
    assert pm["entry_buy_src_other_label"] == "+1.0%×1"
    assert pm["entry_buy_src_other_lots"] == 1
    assert pm["entry_buy_src_none_pct"] == 0.5
    assert pm["entry_buy_src_none_label"] == "+0.5%×1"
    assert pm["entry_buy_src_unknown_lots"] == 1
    assert pm["entry_buy_src_marks_ready"] is True
    assert "scan −0.5%×1" in pm["entry_buy_src_marks_bit"]
    assert "rebal +2.5%×1" in pm["entry_buy_src_marks_bit"]
    assert "other +1.0%×1" in pm["entry_buy_src_marks_bit"]
    assert "none +0.5%×1" in pm["entry_buy_src_marks_bit"]

    empty = entry_buy_source_mark_returns([], [])
    assert empty["entry_buy_src_marks_ready"] is False
    assert empty["entry_buy_src_marks_bit"] == ""

    no_ledger = entry_buy_source_mark_returns(
        [
            {
                "symbol": "META",
                "kind": "stock",
    ...
    assert no_ledger["entry_buy_src_marks_ready"] is False
    assert no_ledger["entry_buy_src_unknown_lots"] == 1
    ...
    assert out["entry_buy_src_marks_ready"] is True
    assert out["entry_buy_src_scan_pct"] == -0.5
    assert out["entry_buy_src_rebal_pct"] == 2.5
    assert "src " not in out["note"]


def test_entry_buy_score_mark_returns_hi_mid_lo_none() -> None:
    from stock_checker.risk_halts import (
        book_risk_report,
        entry_buy_score_mark_returns,
    )
    ...
    pm = entry_buy_score_mark_returns(holds, trades)
    assert pm["entry_buy_score_hi_pct"] == -0.5
    assert pm["entry_buy_score_hi_label"] == "−0.5%×1"
    assert pm["entry_buy_score_hi_lots"] == 1
    assert pm["entry_buy_score_mid_pct"] == 2.5
    assert pm["entry_buy_score_mid_label"] == "+2.5%×1"
    assert pm["entry_buy_score_mid_lots"] == 1
    assert pm["entry_buy_score_lo_pct"] == 1.0
    assert pm["entry_buy_score_lo_label"] == "+1.0%×1"
    assert pm["entry_buy_score_lo_lots"] == 1
    assert pm["entry_buy_score_none_pct"] == 0.5
    assert pm["entry_buy_score_none_label"] == "+0.5%×1"
    assert pm["entry_buy_score_none_lots"] == 1
    assert pm["entry_buy_score_unknown_lots"] == 1
    assert pm["entry_buy_score_marks_ready"] is True
    assert "hi −0.5%×1" in pm["entry_buy_score_marks_bit"]
    assert "mid +2.5%×1" in pm["entry_buy_score_marks_bit"]
    assert "lo +1.0%×1" in pm["entry_buy_score_marks_bit"]
    assert "none +0.5%×1" in pm["entry_buy_score_marks_bit"]
    assert pm["entry_buy_score_hi_floor"] == 50.0
    assert pm["entry_buy_score_mid_floor"] == 25.0

    empty = entry_buy_score_mark_returns([], [])
    assert empty["entry_buy_score_marks_ready"] is False
    assert empty["entry_buy_score_marks_bit"] == ""

    no_ledger = entry_buy_score_mark_returns(
        [
            {
                "symbol": "META",
                "kind": "stock",
    ...
    assert no_ledger["entry_buy_score_marks_ready"] is False
    assert no_ledger["entry_buy_score_unknown_lots"] == 1
    ...
    assert out["entry_buy_score_marks_ready"] is True
    assert out["entry_buy_score_hi_pct"] == -0.5
    assert out["entry_buy_score_mid_pct"] == 2.5
...
def test_entry_buy_size_mark_returns_fat_ok_thin_none() -> None:
    from stock_checker.risk_halts import (
        book_risk_report,
        entry_buy_size_mark_returns,
    )
    ...
    # fat: 2000/(8000+2000)=0.20 · ok: 1000/(9000+1000)=0.10 ·
    # thin: 200/(9800+200)≈0.02 · none: missing cash_remaining
    trades = [
        {
            "type": "BUY",
            "symbol": "WMT",
    ...
    pm = entry_buy_size_mark_returns(holds, trades)
    assert pm["entry_buy_size_fat_pct"] == -0.5
    assert pm["entry_buy_size_fat_label"] == "−0.5%×1"
    assert pm["entry_buy_size_fat_lots"] == 1
    assert pm["entry_buy_size_ok_pct"] == 2.5
    assert pm["entry_buy_size_ok_label"] == "+2.5%×1"
    assert pm["entry_buy_size_ok_lots"] == 1
    assert pm["entry_buy_size_thin_pct"] == 1.0
    assert pm["entry_buy_size_thin_label"] == "+1.0%×1"
    assert pm["entry_buy_size_thin_lots"] == 1
    assert pm["entry_buy_size_none_pct"] == 0.5
    assert pm["entry_buy_size_none_label"] == "+0.5%×1"
    assert pm["entry_buy_size_none_lots"] == 1
    assert pm["entry_buy_size_unknown_lots"] == 1
    assert pm["entry_buy_size_marks_ready"] is True
    assert pm["entry_buy_size_default_frac"] == 0.10
    assert "fat −0.5%×1" in pm["entry_buy_size_marks_bit"]
    assert "ok +2.5%×1" in pm["entry_buy_size_marks_bit"]
    assert "thin +1.0%×1" in pm["entry_buy_size_marks_bit"]
    assert "none +0.5%×1" in pm["entry_buy_size_marks_bit"]

    empty = entry_buy_size_mark_returns([], [])
    assert empty["entry_buy_size_marks_ready"] is False
    assert empty["entry_buy_size_marks_bit"] == ""

    no_ledger = entry_buy_size_mark_returns(
        [
            {
                "symbol": "META",
                "kind": "stock",
    ...
    assert no_ledger["entry_buy_size_marks_ready"] is False
    assert no_ledger["entry_buy_size_unknown_lots"] == 1
    ...
    assert out["entry_buy_size_marks_ready"] is True
    assert out["entry_buy_size_fat_pct"] == -0.5
    assert out["entry_buy_size_ok_pct"] == 2.5
    assert "buy-% " not in out["note"]
